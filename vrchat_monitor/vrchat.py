...
class VRChatClient:
    ...
    def _request(self, method: str, path: str, body: dict[str, Any] | None = None, cookie: str | None = None) -> tuple[Any, str | None]:
        ...
        try:
            with self._open_url(request, timeout=25) as response:
                ...
                return parsed, _cookie_header(response.headers)
    ...
    def login(self, username: str, password: str) -> dict[str, Any]:
        ...
        if parsed.get("requiresTwoFactorAuth"):
            ...
            self._pending_user = parsed
            return {"requires_2fa": True, "methods": parsed.get("requiresTwoFactorAuth", ["totp"])}
        if not cookie:
            raise VRChatError("登录响应没有返回会话，请稍后重试")
        self._finish_login(parsed, cookie)
        return {"requires_2fa": False, "user": parsed}

    def verify_2fa(self, code: str) -> dict[str, Any]:
        cookie = getattr(self, "_pending_cookie", "")
        if not cookie:
            raise VRChatError("登录会话已过期，请重新登录", 401)
        parsed, new_cookie = self._request("POST", "/auth/twofactorauth/totp/verify", {"code": code}, cookie)
        cookie = "; ".join(filter(None, [cookie, new_cookie]))
        self._finish_login(parsed, cookie)
        return parsed
    ...
    def start_pipeline(self, on_event: Callable[[dict[str, Any]], None], stop_event: threading.Event) -> threading.Thread:
        thread = threading.Thread(target=self._pipeline_loop, args=(on_event, stop_event), name="vrchat-pipeline", daemon=True)
        thread.start()
        return thread

    def _pipeline_loop(self, on_event: Callable[[dict[str, Any]], None], stop_event: threading.Event) -> None:
        ...
        while not stop_event.is_set() and self.cookie:
            try:
                ...
                stop_event.wait(5 + random.random() * 5)
    ...
    def _pipeline_once(self, on_event: Callable[[dict[str, Any]], None], stop_event: threading.Event) -> None:
        ...
        try:
            sock.settimeout(30)
            request = (
                f"GET /?authToken={urllib.parse.quote(self.cookie or '')} HTTP/1.1\r\n"
            ...
            if b" 101 " not in response.split(b"\r\n", 1)[0]:
                raise VRChatError("WebSocket 握手失败")
    ...
    def _read_frame(sock: ssl.SSLSocket) -> tuple[int, bytes]:
        ...
        while len(payload) < length:
            payload.extend(sock.recv(min(65536, length - len(payload))))
        if masked:
